...
def page(path=''):
    ...
    if '/packages/' in path and path.endswith('dart'):
        ...
        return bottle.HTTPResponse(
            body=(Path('packages') / relpath).read_text(),
            content_type='application/dart'
        )
    ...
    if file_ is None:
        return bottle.HTTPResponse(
            status=404,
            body=(Path('web') / '404.html').read_text()
        )
...
def build():
    clean()
    # build_dart()
    for src in Path('web').rglob('*?.*'):
        dest = Path('build') / src.relative_to('web')
        # Ignore files generated by dart2js.
        if dest.suffix in ('.dart', '.map', '.deps'):
            continue
        print dest
...
# @task
# def build_dart():
#     for src in Path('web').rglob('*app.dart'):
#         dest = src.parent / 'app.js'
#         cmd = 'dart2js -m -o %s %s' % (dest, src)
#         print cmd
#         run(cmd)
